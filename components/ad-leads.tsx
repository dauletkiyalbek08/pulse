...
/** Лиды с рекламы (квиз/сайт) за 30 дней — поимённо, кто пришёл. */
export function AdLeads({ leads }: { leads: AdLead[] }) {
  return (
    <div className="rounded-card bg-surface p-5 shadow-soft ring-1 ring-line">
      <div className="mb-3 flex items-center gap-2">
        ...
        <div>
          <div className="text-sm font-semibold text-ink">Лиды с рекламы (квиз) · 30 дней</div>
          <div className="text-xs text-muted">Люди, оставившие заявку через квиз/сайт. Купившие — вверху.</div>
        </div>
      </div>

      {leads.length === 0 ? (
        <p className="rounded-lg bg-canvas px-3 py-4 text-center text-sm text-muted">
          Пока нет заявок с рекламы за 30 дней.
        </p>
      ) : (
